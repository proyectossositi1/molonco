...
class CatTipoPagoController extends BaseController
{
    ...
    public function store(){
        ...
        if($data->nombre != ""){
            ...
            if(!empty($encontrado)){
                ...
                if($encontrado['id'] != $id && $encontrado['nombre'] != $data->nombre){
                    ...
                    $response['response_message'] = [
                        'type' => 'warning',
                        'message' => 'EL TIPO DE PAGO '.$data->nombre.' YA SE ENCUENTRA REGISTRADO. INTENTE CON OTRO NOMBRE.'
                    ];
                }
            }
            ...
            if($next){
                ...
                if($id != ""){                
                    ...
                    if ($model->where('nombre', $data->nombre)->countAllResults() > 0) {
                        $response['response_message'] = [
                            'type' => 'error',
                            'message' => 'EL TIPO DE PAGO YA SE ENCUENTRA AGREGADA. INTENTE CON OTRO NOMBRE.'
                        ];
                    }else{
                        ...
                        $response['response_message'] = [
                            'type' => 'success',
                            'message' => 'SE AGREGO CON EXITO EL TIPO DE PAGO.'
                        ];  
                    }                                  
                } 
            }
            ...
            $response['view'] =  view('catalogos/tipopagos/ajax/table_data', $data_view);
        }
